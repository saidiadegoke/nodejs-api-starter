...
const getPlatformStats = async () => {
  const result = await pool.query(`
    SELECT 
      ...
      (SELECT COUNT(*) FROM comments WHERE created_at >= NOW() - INTERVAL '30 days') as new_comments_30d,
      (SELECT COUNT(*) FROM comments WHERE created_at >= NOW() - INTERVAL '7 days') as new_comments_7d,
...
const getUserEngagement = async (timeframe = '30d') => {
  ...
  const result = await pool.query(`
    WITH user_activities AS (
      SELECT user_id, created_at FROM poll_responses WHERE created_at >= NOW() - INTERVAL $1
      UNION ALL
      SELECT created_by as user_id, created_at FROM polls WHERE created_at >= NOW() - INTERVAL $1
      UNION ALL
      SELECT created_by as user_id, created_at FROM context_sources WHERE created_at >= NOW() - INTERVAL $1
      UNION ALL
      SELECT created_by as user_id, created_at FROM comments WHERE created_at >= NOW() - INTERVAL $1
    ),
    user_stats AS (
      SELECT 
        user_id,
...
const getPollPerformance = async (timeframe = '30d') => {
  ...
  const result = await pool.query(`
    WITH poll_stats AS (
      ...
      WHERE p.created_at >= NOW() - INTERVAL $1
  ...
  const result = await pool.query(`
    WITH content_stats AS (
      ...
      SELECT 
        ...
        cs.id,
        cs.created_by,
        CHAR_LENGTH(cs.content) as content_length,
        cs.created_at
      FROM context_sources cs
      WHERE cs.created_at >= NOW() - INTERVAL $1
      ...
      SELECT 
        ...
        c.id,
        c.created_by,
        CHAR_LENGTH(c.content) as content_length,
        c.created_at
      FROM comments c
...
const getTrendingTopics = async (timeframe = '30d') => {
  ...
  const result = await pool.query(`
    ...
    SELECT 
      ...
      poll_mentions,
      story_mentions,
      ROUND((frequency::float / (SELECT SUM(frequency) FROM word_counts)) * 100, 2) as percentage
    FROM word_counts
    WHERE frequency >= 3
  ...
  const result = await pool.query(`
    WITH date_series AS (
      SELECT generate_series(
        date_trunc(CASE WHEN $2 = '7d' THEN 'day' ELSE 'month' END, NOW() - INTERVAL $1),
        date_trunc(CASE WHEN $2 = '7d' THEN 'day' ELSE 'month' END, NOW()),
        CASE WHEN $2 = '7d' THEN '1 day'::interval ELSE '1 month'::interval END
      ) as period
    ),
    growth_data AS (
      SELECT 
        date_trunc(CASE WHEN $2 = '7d' THEN 'day' ELSE 'month' END, created_at) as period,
        'users' as metric_type,
        COUNT(*) as value
      FROM users
      WHERE created_at >= NOW() - INTERVAL $1
      GROUP BY date_trunc(CASE WHEN $2 = '7d' THEN 'day' ELSE 'month' END, created_at)
  ...
  const result = await pool.query(`
    ...
    story_performance AS (
      SELECT 
        ...
        COUNT(DISTINCT c.created_by) as unique_users,
        u.first_name || ' ' || u.last_name as creator_name
      FROM context_sources cs
      LEFT JOIN comments c ON c.context_source_id = cs.id
  ...
  const result = await pool.query(`
    ...
    user_activities AS (
      ...
      FROM (
        ...
        UNION ALL
        SELECT created_by as user_id, created_at FROM polls
        UNION ALL
        SELECT created_by as user_id, created_at FROM context_sources
        UNION ALL
        SELECT created_by as user_id, created_at FROM comments
      ) activities
